...
public class ClusterConfigBuilder {
  ...
  /**
   * When chunking is enabled, records must exceed this threshold to trigger actual chunking.
   * {@link #CHUNKING_TEST_RECORD_SIZE} is set larger than this so test records get chunked.
   */
  public static final int CHUNKING_TRIGGER_THRESHOLD_BYTES = 200;

  /**
   * Minimum record value size used by BatchPushExecutor when chunking is enabled.
   * Must be larger than {@link #CHUNKING_TRIGGER_THRESHOLD_BYTES} to ensure records get chunked.
   */
  public static final int CHUNKING_TEST_RECORD_SIZE = 1024;
  ...
  public static Properties buildServerProperties(TestCaseConfig config) {
    ...
    // When chunking is enabled, reduce the chunk size threshold so that test records
    // actually get chunked. Without this, chunking is enabled at the store level but
    // never exercised because test records are too small to exceed the default ~950KB
    // threshold. BatchPushExecutor.writeTestInputData() generates records of
    // CHUNKING_TEST_RECORD_SIZE bytes when chunking is enabled.
    if (config.isChunking()) {
      props.setProperty(
          VeniceWriter.MAX_SIZE_FOR_USER_PAYLOAD_PER_MESSAGE_IN_BYTES,
          String.valueOf(CHUNKING_TRIGGER_THRESHOLD_BYTES));
    }
