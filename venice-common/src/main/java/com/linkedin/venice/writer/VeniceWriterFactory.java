...
public class VeniceWriterFactory {
   ...
   */
  public VeniceWriter<byte[], byte[], byte[]> getBasicVeniceWriter(String topicName, Time time) {
    return getVeniceWriter(topicName, new DefaultSerializer(), new DefaultSerializer(), new DefaultSerializer(),
        Optional.empty(), time);
  }

  public VeniceWriter<byte[], byte[], byte[]> getBasicVeniceWriter(String topicName) {
   ...
   */
  public VeniceWriter<byte[], byte[], byte[]> getBasicVeniceWriter(String topicName, boolean chunkingEnabled) {
    return getVeniceWriter(topicName, new DefaultSerializer(), new DefaultSerializer(), new DefaultSerializer(),
        Optional.of(chunkingEnabled), SystemTime.INSTANCE);
  }

  public <K, V> VeniceWriter<K, V, byte[]> getVeniceWriter(String topicName, VeniceKafkaSerializer<K> keySerializer,
      VeniceKafkaSerializer<V> valueSerializer) {
    return getVeniceWriter(topicName, keySerializer, valueSerializer, new DefaultSerializer(),
   ...
   * @param chunkingEnabled override the factory's default chunking setting.
   */
  protected <K, V, U> VeniceWriter<K, V, U> getVeniceWriter(String topic, VeniceKafkaSerializer<K> keySerializer,
  ...
  public VeniceWriter<KafkaKey, byte[], byte[]> getVeniceWriter(String topic) {
    return getVeniceWriter(topic, new KafkaKeySerializer(), new DefaultSerializer(), new DefaultSerializer(),
        Optional.empty(), SystemTime.INSTANCE);
  }
