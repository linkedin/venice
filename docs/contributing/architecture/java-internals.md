...
2. Objects can be used as a type parameter in a class which has
   ...
   example, a `List<Integer>` is valid, while a `List<int>` is not. That being said, in the case of common collections,
   ...
   implementation of `List<Integer>` with additional methods that prevent "boxing" (see below).
