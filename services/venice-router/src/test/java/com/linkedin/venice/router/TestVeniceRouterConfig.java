...
public class TestVeniceRouterConfig {
  ...
  public void helixGroupLatencyKnobsAreOverridable() {
    VeniceProperties props = getPropertyBuilderWithBasicConfigsFilledIn()
        ...
        .put(ROUTER_HELIX_GROUP_EVEN_UNTIL_LATENCY_MS, 1.5)
        .put(ROUTER_HELIX_GROUP_FULL_SKEW_AT_LATENCY_MS, 3.0)
    ...
    assertEquals(config.getHelixGroupEvenUntilLatencyMs(), 1.5);
    assertEquals(config.getHelixGroupFullSkewAtLatencyMs(), 3.0);
