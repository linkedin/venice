...
public class StoreReadThrottlerTest {
  ...
  public void testBuildAndUpdateStoreReadThrottler() {
    ...
    helixStateToInstancesMapForP0.put(HelixState.LEADER, Collections.singletonList(instance1));
    helixStateToInstancesMapForP0.put(HelixState.STANDBY, Collections.singletonList(instance2));
    ...
    helixStateToInstancesMapForP1.put(HelixState.LEADER, Collections.singletonList(instance1));
    helixStateToInstancesMapForP1.put(HelixState.STANDBY, Collections.singletonList(instance2));
    EnumMap<ExecutionStatus, List<Instance>> executionStatusToInstancesMapForP1 = new EnumMap<>(ExecutionStatus.class);
    executionStatusToInstancesMapForP1.put(ExecutionStatus.COMPLETED, Collections.singletonList(instance1));
    executionStatusToInstancesMapForP1.put(ExecutionStatus.STARTED, Collections.singletonList(instance2));
    ...
    // Instance 2 hold 1 of 2 online replicas for partition 0 and 0 of 1 online replicas for partition 1.
    Assert.assertEquals(
    ...
    // Instance 2 hold 1 of 2 online replicas for partition 0 and 0 of 0 online replicas for partition 1.
    Assert.assertEquals(
        throttler.getQuotaForStorageNode(Utils.getHelixNodeIdentifier(Utils.getHostName(), 10002)),
        (long) (quota / (double) partitionCount / 2 * (1 + perStorageNodeReadQuotaBuffer)));
  }
  ...
  public void testThrottle() {
    ...
    for (int i = 0; i < partitionCount; i++) {
      ...
      helixStateToInstancesMap.put(HelixState.LEADER, Collections.singletonList(instances[0]));
    ...
    StoreReadThrottler throttler =
        new StoreReadThrottler(storeName, quota, EventThrottler.REJECT_STRATEGY, 0.0, 1000, 1000);
    throttler.mayThrottleRead(400);
    try {
      throttler.mayThrottleRead(100);
