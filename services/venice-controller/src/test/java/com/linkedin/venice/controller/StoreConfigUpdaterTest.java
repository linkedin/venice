...
public class StoreConfigUpdaterTest extends AbstractTestVeniceParentHelixAdmin {
  ...
  /**
   * Covers the race-fix: when {@code targetRegionPromotedVersionNum} is set in params, it must be
   * propagated verbatim into the {@code UpdateStore} Avro message so child controllers can use the
   * exact version number instead of falling back to {@code getLargestUsedVersionNumber()}.
   */
  @Test
  public void testApplyOnParent_TargetRegionPromotedVersionNum_AppearsInAvro() {
    String storeName = Utils.getUniqueString("parent-trp-vnum");
    Store store = TestUtils.createTestStore(storeName, "test-owner", System.currentTimeMillis());
    doReturn(store).when(internalAdmin).getStore(clusterName, storeName);
    parentAdmin.initStorageCluster(clusterName);

    int expectedVersionNum = 42;
    UpdateStoreQueryParams params = new UpdateStoreQueryParams().setTargetRegionPromoted(true)
        .setTargetRegionPromotedVersionNum(expectedVersionNum);
    parentAdmin.updateStore(clusterName, storeName, params);

    UpdateStore msg = captureLastUpdateStore();
    assertTrue(msg.targetRegionPromoted, "Avro field targetRegionPromoted must be true");
    org.testng.Assert.assertEquals(
        (int) msg.targetRegionPromotedVersionNum,
        expectedVersionNum,
        "targetRegionPromotedVersionNum must be propagated verbatim to the Avro message");
  }

  /**
   * Covers the backward-compat default: when {@code targetRegionPromotedVersionNum} is NOT set in
   * params, the Avro message must carry {@code -1} (the sentinel for "fall back to
   * getLargestUsedVersionNumber() on the child").
   */
  @Test
  public void testApplyOnParent_TargetRegionPromotedWithoutVersionNum_DefaultsToMinusOne() {
    String storeName = Utils.getUniqueString("parent-trp-no-vnum");
    Store store = TestUtils.createTestStore(storeName, "test-owner", System.currentTimeMillis());
    doReturn(store).when(internalAdmin).getStore(clusterName, storeName);
    parentAdmin.initStorageCluster(clusterName);
    ...
    org.testng.Assert.assertEquals(
        (int) msg.targetRegionPromotedVersionNum,
        -1,
        "targetRegionPromotedVersionNum must default to -1 when not explicitly set");
  }

  /**
   * Covers the child dispatch: when {@code targetRegionPromoted=true} and
   * {@code targetRegionPromotedVersionNum} is supplied, {@code storeMetadataUpdate} must still be
   * invoked (the lambda will use the explicit version rather than getLargestUsedVersionNumber).
   */
  @Test
  public void testApplyOnChild_TargetRegionPromotedWithVersionNum_CallsMetadataUpdate() {
    String storeName = Utils.getUniqueString("child-trp-vnum");
    VeniceHelixAdmin admin = newChildAdminMock(storeName);

    UpdateStoreQueryParams params =
        new UpdateStoreQueryParams().setTargetRegionPromoted(true).setTargetRegionPromotedVersionNum(7);
