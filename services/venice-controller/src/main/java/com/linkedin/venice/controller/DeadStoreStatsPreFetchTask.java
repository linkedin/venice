...
public class DeadStoreStatsPreFetchTask implements Runnable, Closeable {
  ...
  public DeadStoreStatsPreFetchTask(String clusterName, VeniceParentHelixAdmin admin, long refreshIntervalMs) {
  ...
  public void run() {
    ...
    isRunning.set(true);
    admin.deadStoreStats.preFetchStats(getStoresInCluster()); // Fetch before cycling
    while (isRunning.get()) {
      try {
        Utils.sleep(refreshIntervalMs);
        long startTime = System.currentTimeMillis();
        logger.debug("Fetching dead store stats for cluster: {}", clusterName);
        admin.deadStoreStats.preFetchStats(getStoresInCluster());
