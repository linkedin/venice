...
public class TopicCleanupService extends AbstractVeniceService {
  ...
  void cleanupVeniceTopics() {
    ...
    // topics
    // this will save a lot of time wasted in sorting
    ...
    while (!allTopics.isEmpty()) {
      ...
      try {
        String cluster = admin.discoverCluster(storeName).getFirst();
        if (topic.isRealTime() && admin.canDeleteRTTopic(cluster, storeName)) {
