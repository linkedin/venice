...
public enum Command {
  ...
  ENABLE_STORE("enable-store", "Enable a store in both read and write path",
      new Arg[] {URL, CLUSTER, STORE}),
  JOB_STATUS("job-status", "Query the ingest status of a running push job. If a version is not specified, the job status of the last job will be printed.",
  ...
  ALLOW_LIST_ADD_NODE("allow-list-add-node", "Add a storage node into the allowlist",
      new Arg[]{URL, CLUSTER, STORAGE_NODE}),
  ALLOW_LIST_REMOVE_NODE("allow-list-remove-node", "Remove a storage node from the allowlist",
      new Arg[]{URL, CLUSTER, STORAGE_NODE}),
  ...
  START_MIRROR_MAKER("start-kafka-mirror-maker",
      ...
      new Arg[]{KAFKA_ZOOKEEPER_CONNECTION_URL_SOURCE, KAFKA_ZOOKEEPER_CONNECTION_URL_DESTINATION,
          KAFKA_BOOTSTRAP_SERVERS_DESTINATION, KAFKA_TOPIC_ALLOWLIST},
  ...
  END_MIGRATION("end-migration", "Send this command to delete the original store",
      new Arg[] {URL, STORE, CLUSTER_SRC, CLUSTER_DEST}),
  SEND_END_OF_PUSH("send-end-of-push", "Send this message after Samza reprocessing job to close offline batch push",
      new Arg[] {URL, CLUSTER, STORE, VERSION}),
  ...
  LIST_CLUSTER_STALE_STORES("list-cluster-stale-stores", "List all stores in a cluster which have stale replicas.",
      new Arg[] {URL, CLUSTER}, new Arg[] {REGIONS_FILTER}),
