...
public class CollectionOperationSequenceBuilder {
  ...
  private List<List<CollectionOperation>> createOperationSequence(List<List<CollectionOperation>> allColoOperationSequences) {
    int totalOpCount = 0;
    ...
    for (int i = 0; i < allColoOperationSequences.size(); i++) {
      createOperationSequenceHelper(allColoOperationSequences, i, totalOpCount, new LinkedList<>(), allResults, new HashSet<>());
    }
    return allResults;
  }
  ...
  private void createOperationSequenceHelper(
    ...
    if (ops.isEmpty()) {
      createOperationSequenceHelper(allColoOperationSequences, currLevel + 1, totalOpCount, currResultOps, allResults, allResultsDedupSet);
      return;
    }
    // ops is not empty.
