...
class IngestionNotificationDispatcher {
        ...
        notifier -> {
          ...
          if (pcs.isErrorReported()) {
            ...
            logger.error("Processing completed WITH ERRORS for Partition: " + pcs.getUserPartition() +
                ", Last Offset: " + pcs.getLatestProcessedLocalVersionTopicOffset());
            return false;
          }
  ...
  void reportQuotaNotViolated(PartitionConsumptionState pcs) {
    report(pcs, HybridStoreQuotaStatus.QUOTA_NOT_VIOLATED,
        notifier -> notifier.quotaNotViolated(topic, pcs.getUserPartition(), pcs.getLatestProcessedLocalVersionTopicOffset()),
  ...
  void reportQuotaViolated(PartitionConsumptionState pcs) {
    report(pcs, HybridStoreQuotaStatus.QUOTA_VIOLATED,
        notifier -> notifier.quotaViolated(topic, pcs.getUserPartition(), pcs.getLatestProcessedLocalVersionTopicOffset()),
  ...
  void reportProgress(PartitionConsumptionState pcs) {
    report(pcs, ExecutionStatus.PROGRESS,
        notifier -> notifier.progress(topic, pcs.getUserPartition(), pcs.getLatestProcessedLocalVersionTopicOffset()),
        () -> {
  ...
  void reportEndOfPushReceived(PartitionConsumptionState pcs) {
    report(pcs, ExecutionStatus.END_OF_PUSH_RECEIVED,
        notifier -> notifier.endOfPushReceived(topic, pcs.getUserPartition(), pcs.getLatestProcessedLocalVersionTopicOffset()));
  }
  ...
  void reportEndOfIncrementalPushReceived(PartitionConsumptionState pcs, String version) {
    report(pcs, ExecutionStatus.END_OF_INCREMENTAL_PUSH_RECEIVED,
        notifier ->
            notifier.endOfIncrementalPushReceived(topic, pcs.getUserPartition(), pcs.getLatestProcessedLocalVersionTopicOffset(),
  ...
  void reportStartOfBufferReplayReceived(PartitionConsumptionState pcs) {
    report(pcs, ExecutionStatus.START_OF_BUFFER_REPLAY_RECEIVED,
        notifier -> notifier.startOfBufferReplayReceived(topic, pcs.getUserPartition(), pcs.getLatestProcessedLocalVersionTopicOffset()));
  }

  void reportTopicSwitchReceived(PartitionConsumptionState pcs) {
    report(pcs, ExecutionStatus.TOPIC_SWITCH_RECEIVED,
        notifier -> notifier.topicSwitchReceived(topic, pcs.getUserPartition(), pcs.getLatestProcessedLocalVersionTopicOffset()));
  }
  ...
  void reportStopped(PartitionConsumptionState pcs) {
    report(pcs, "STOPPED",
        notifier -> {
          notifier.stopped(topic, pcs.getUserPartition(), pcs.getLatestProcessedLocalVersionTopicOffset());
        },
        () -> true);
  }
