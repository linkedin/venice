...
class MergeGenericRecord implements Merge<GenericRecord> {
  ...
  public ValueAndReplicationMetadata<GenericRecord> delete(
      ...
      long writeOperationTimestamp, long sourceOffsetOfNewValue, int sourceBrokerIDOfNewValue) {
    GenericRecord oldTimestampMetadata = oldValueAndReplicationMetadata.getReplicationMetadata();

    Object tsObject = oldTimestampMetadata.get(TIMESTAMP_FIELD);
    TSMDType tsmdType = Merge.getTSMDType(tsObject);

    // Always update the vector field
    oldTimestampMetadata.put(REPLICATION_CHECKPOINT_VECTOR_FIELD,
        Merge.mergeOffsetVectors((List<Long>)oldTimestampMetadata.get(REPLICATION_CHECKPOINT_VECTOR_FIELD), sourceOffsetOfNewValue, sourceBrokerIDOfNewValue));
