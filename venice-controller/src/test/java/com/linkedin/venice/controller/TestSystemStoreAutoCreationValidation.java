...
public class TestSystemStoreAutoCreationValidation {
  ...
  public void testThrowExceptionWhenAutoCreationFailed() {
    ...
    when(mockHelixAdmin.incrementVersionIdempotent(anyString(), anyString(), anyString(), anyInt(), anyInt())).thenReturn(mockNewVersion);
    doCallRealMethod().when(mockHelixAdmin).validateAndMaybeRetrySystemStoreAutoCreation(anyString(), anyString(), any());

    mockHelixAdmin.validateAndMaybeRetrySystemStoreAutoCreation("testCluster", testStoreName, VeniceSystemStoreType.META_STORE);
    ...
    veniceStore.setSystemStores(Collections.singletonMap(VeniceSystemStoreType.META_STORE.getPrefix(), systemStoreAttributes));
    String expectedExceptionMessage = "System store:" + systemStoreName + " push is still ongoing, will check it again. This is not an error.";
    assertThrowsWithExceptionMessage(() -> mockHelixAdmin.validateAndMaybeRetrySystemStoreAutoCreation("testCluster", testStoreName, VeniceSystemStoreType.META_STORE), expectedExceptionMessage);

    // Set added system store version to ERROR state to simulate and verify that method will do empty push and throw exception.
    version.setStatus(VersionStatus.ERROR);
    expectedExceptionMessage = "System store: " + systemStoreName + " pushed failed. Issuing a new empty push to create version: 2";
    assertThrowsWithExceptionMessage(() -> mockHelixAdmin.validateAndMaybeRetrySystemStoreAutoCreation("testCluster", testStoreName, VeniceSystemStoreType.META_STORE), expectedExceptionMessage);
