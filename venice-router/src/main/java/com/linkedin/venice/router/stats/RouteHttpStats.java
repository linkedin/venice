...
public class RouteHttpStats {
  ...
  public void recordResponseWaitingTime(String hostName, double waitingTime) {
    InternalRouteHttpStats
        routeStats = routeStatsMap.computeIfAbsent(hostName, h -> new InternalRouteHttpStats(metricsRepository, h, requestType));
  ...
  class InternalRouteHttpStats extends AbstractVeniceHttpStats {
    ...
    public InternalRouteHttpStats(MetricsRepository metricsRepository, String hostName, RequestType requestType) {
      ...
      responseWaitingTimeSensor = registerSensor("response_waiting_time",
          TehutiUtils.getPercentileStat(getName(), getFullMetricName("response_waiting_time")), new Max());
    }
