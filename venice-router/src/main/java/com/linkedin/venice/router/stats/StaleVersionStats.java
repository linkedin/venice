...
public class StaleVersionStats extends AbstractVeniceStats {
  ...
  private final VeniceConcurrentHashMap<StaleVersionReason, Sensor> staleVersionReasonStats = new VeniceConcurrentHashMap<>();
  ...
  public StaleVersionStats(MetricsRepository metricsRepository, String name) {
    ...
    for (StaleVersionReason reason : StaleVersionReason.values()) {
        staleVersionReasonStats.computeIfAbsent(reason,
            staleVersionReason -> registerSensor("stale_version_reason_" + staleVersionReason.name().toLowerCase(), new OccurrenceRate()));
    }
  }
