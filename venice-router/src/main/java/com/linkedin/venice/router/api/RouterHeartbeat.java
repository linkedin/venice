...
public class RouterHeartbeat extends AbstractVeniceService {
  ...
  public RouterHeartbeat(Map<Instance, CloseableHttpAsyncClient> clientPool, VeniceHostHealth health){
    this(clientPool, health, 10L, TimeUnit.SECONDS);
  }

  public RouterHeartbeat(Map<Instance, CloseableHttpAsyncClient> clientPool, VeniceHostHealth health, long cycleTime, TimeUnit cycleTimeUnits){
    Runnable runnable = () -> {
      try {
        for (Instance instance : clientPool.keySet()) {
          ...
          try {
            HttpResponse response = client.execute(get, null).get(1, TimeUnit.SECONDS);
