 ...
 *
 * This class implementation is copied from the {@code org.iq80.snappy.Crc32C} implementation
 * because it was stupidly made package private for no good reason. Note that Java9 will provide
 * its own implementation of CRC32-C which may have scope for native acceleration.
 */
...
public final class Crc32C implements Checksum {
  ...
  public void update(byte[] b, int off, int len) {
    ...
    while (len > 7) {
      ...
      int c1 = b[off++] ^ (localCrc >>>= 8); // SUPPRESS CHECKSTYLE InnerAssignment
      int c2 = b[off++] ^ (localCrc >>>= 8); // SUPPRESS CHECKSTYLE InnerAssignment
      int c3 = b[off++] ^ (localCrc >>>= 8); // SUPPRESS CHECKSTYLE InnerAssignment
