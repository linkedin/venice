...
public class TestEspressoHttpObjectAggregator {
  @Test
  public void testAggregate() {
    EspressoHttpObjectAggregator aggr = new EspressoHttpObjectAggregator(1024 * 1024);
  ...
  public void testOversizedRequest() {
    final EmbeddedChannel embedder = new EmbeddedChannel(new EspressoHttpObjectAggregator(4));
  ...
  public void testOversizedRequestWithContentLengthAndDecoder() {
    EmbeddedChannel embedder =
        new EmbeddedChannel(new HttpRequestDecoder(), new EspressoHttpObjectAggregator(4, false));
    assertFalse(
        embedder.writeInbound(
            Unpooled.copiedBuffer("PUT /upload HTTP/1.1\r\n" + "Content-Length: 5\r\n\r\n", CharsetUtil.US_ASCII)));
    ...
    assertTrue(response instanceof LastHttpContent);
  ...
  private static void assertArrayEquals(byte[] bytes, byte[] actual) {
    assertNotNull(bytes);
    assertNotNull(actual);
    assertEquals(bytes.length, actual.length);
    for (int i = 0; i < bytes.length; i++) {
      assertEquals(bytes[i], actual[i]);
    }
  }
  ...
  public void testInvalidConstructorUsage() {
    assertThrows(IllegalArgumentException.class, () -> new EspressoHttpObjectAggregator(-1)
  ...
  public void testInvalidMaxCumulationBufferComponents() {
    final EspressoHttpObjectAggregator aggr = new EspressoHttpObjectAggregator(Integer.MAX_VALUE);
    assertThrows(IllegalArgumentException.class, () -> aggr.setMaxCumulationBufferComponents(1));
  }
  ...
  public void testBadRequest() {
    EmbeddedChannel ch = new EmbeddedChannel(new HttpRequestDecoder(), new EspressoHttpObjectAggregator(1024 * 1024));
  ...
  public void testBadResponse() throws Exception {
    EmbeddedChannel ch = new EmbeddedChannel(new HttpResponseDecoder(), new EspressoHttpObjectAggregator(1024 * 1024));
  ...
  public void testOversizedRequestWith100Continue() {
    EmbeddedChannel embedder = new EmbeddedChannel(new EspressoHttpObjectAggregator(8));
  ...
  private static void runUnsupportedExceptHeaderExceptionTest(final boolean close) {
    final EspressoHttpObjectAggregator aggregator;
    final int maxContentLength = 4;
    if (close) {
      aggregator = new EspressoHttpObjectAggregator(maxContentLength, true);
    } else {
      aggregator = new EspressoHttpObjectAggregator(maxContentLength);
    }
  ...
  public void testValidRequestWith100ContinueAndDecoder() {
    EmbeddedChannel embedder = new EmbeddedChannel(new HttpRequestDecoder(), new EspressoHttpObjectAggregator(100));
    embedder.writeInbound(
        Unpooled.copiedBuffer(
            "GET /upload HTTP/1.1\r\n" + "Expect: 100-continue\r\n" + "Content-Length: 0\r\n\r\n",
            CharsetUtil.US_ASCII));
  ...
  public void testOversizedRequestWith100ContinueAndDecoder() {
    EmbeddedChannel embedder = new EmbeddedChannel(new HttpRequestDecoder(), new EspressoHttpObjectAggregator(4));
    embedder.writeInbound(
        Unpooled.copiedBuffer(
            "PUT /upload HTTP/1.1\r\n" + "Expect: 100-continue\r\n" + "Content-Length: 100\r\n\r\n",
            CharsetUtil.US_ASCII));
  ...
  public void testOversizedRequestWith100ContinueAndDecoderCloseConnection() {
    EmbeddedChannel embedder = new EmbeddedChannel(new HttpRequestDecoder(), new EspressoHttpObjectAggregator(4, true));
    embedder.writeInbound(
        Unpooled.copiedBuffer(
            "PUT /upload HTTP/1.1\r\n" + "Expect: 100-continue\r\n" + "Content-Length: 100\r\n\r\n",
            CharsetUtil.US_ASCII));
  ...
  public void testRequestAfterOversized100ContinueAndDecoder() {
    EmbeddedChannel embedder = new EmbeddedChannel(new HttpRequestDecoder(), new EspressoHttpObjectAggregator(15));
  ...
  public void testReplaceAggregatedRequest() {
    EmbeddedChannel embedder = new EmbeddedChannel(new EspressoHttpObjectAggregator(1024 * 1024));
  ...
  public void testSelectiveRequestAggregation() {
    EspressoHttpObjectAggregator myPostAggregator = new EspressoHttpObjectAggregator(1024 * 1024) {
      @Override
  ...
  public void testSelectiveResponseAggregation() {
    EspressoHttpObjectAggregator myTextAggregator = new EspressoHttpObjectAggregator(1024 * 1024) {
      @Override
      protected boolean isStartMessage(HttpObject msg) throws Exception {
