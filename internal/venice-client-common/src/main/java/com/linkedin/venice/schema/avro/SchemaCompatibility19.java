...
/**
 * Copy of the original Apache Avro 1.9.2 SchemaCompatibility class from here:
 * https://raw.githubusercontent.com/apache/avro/refs/tags/release-1.9.2/lang/java/avro/src/main/java/org/apache/avro/SchemaCompatibility.java
 *
 * The reason to maintain two versions of SchemaCompatibility is that not all the Venice users
 * are using Avro-1.9+, and so far, we only want to apply the advanced features of SchemaCompatibility
 * on-demand for the users who are using Avro-1.9+.
 * Once all the Venice users upgrade to Avro-1.9+, we can remove {@link com.linkedin.venice.schema.avro.SchemaCompatibility}
 * and use this one by default.
 */
