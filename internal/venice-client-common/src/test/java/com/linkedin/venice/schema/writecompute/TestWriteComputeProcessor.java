...
public class TestWriteComputeProcessor {
  ...
  public void testMergeTwoMapUpdate() {
    ...
    updateV2 = new UpdateBuilderImpl(UPDATE_SCHEMA).setNewFieldValue("name", "abc")
        ...
        .build();
    resultUpdate = updateHandler.mergeUpdateRecord(VALUE_SCHEMA, updateV1, updateV2);
    LogManager.getLogger(TestWriteComputeProcessor.class).info("DEBUGGING: {}", resultUpdate);

    Map<String, Integer> updatedField = (Map<String, Integer>) resultUpdate.get("IntMapField");
    Assert.assertEquals(updatedField.size(), 3);
    Assert.assertEquals((int) updatedField.get("a"), 1);
  ...
  public void testMergeTwoListUpdate() {
    ...
    Assert.assertTrue(unionList.contains("abc"));
    Assert.assertTrue(unionList.contains("def"));
    Assert.assertTrue(unionList.contains("xyz"));
    Assert.assertTrue(unionList.contains("ghi"));
    Assert.assertTrue(diffList.contains("xyz"));
