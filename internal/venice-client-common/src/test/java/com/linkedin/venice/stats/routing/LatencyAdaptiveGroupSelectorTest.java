...
public class LatencyAdaptiveGroupSelectorTest {
  ...
  public void testInvalidKnobsThrow() {
    ...
    // evenUntil == fullSkew
    assertThrows(VeniceException.class, () -> new LatencyAdaptiveGroupSelector(lat, 1.5, 1.5, 1.0, PICK_FIRST));
    // evenUntil < 1
    assertThrows(VeniceException.class, () -> new LatencyAdaptiveGroupSelector(lat, 0.9, 2.0, 1.0, PICK_FIRST));
    // interpolation exponent == 0
    assertThrows(VeniceException.class, () -> new LatencyAdaptiveGroupSelector(lat, 1.2, 2.0, 0.0, PICK_FIRST));
    // interpolation exponent < 0
    assertThrows(VeniceException.class, () -> new LatencyAdaptiveGroupSelector(lat, 1.2, 2.0, -1.0, PICK_FIRST));
  }
  ...
  public void testUnmeasuredGroupsStayEven() {
    ...
    LatencyAdaptiveGroupSelector selector =
        new LatencyAdaptiveGroupSelector(latencies(-1.0, -1.0, -1.0), 1.2, 2.0, 1.0, PICK_FIRST);
  ...
  public void testExcludedGroupNeverSelected() {
    ...
    LatencyAdaptiveGroupSelector selector =
        new LatencyAdaptiveGroupSelector(latencies(5.0, 5.0, 5.0), 1.2, 2.0, 1.0, cycling);
  ...
  @Test
  public void testExcludedSlowGroupDoesNotDistortSpread() {
    // group0 = 10ms, group1 = 10ms, group2 = 500ms. Without exclusion the 50x spread forces full skew off group2.
    // Excluding group2 leaves two equal groups => skew 0 => even split between 0 and 1, and group2 never appears.
    Random random = new Random(11);
    LatencyAdaptiveGroupSelector selector =
        new LatencyAdaptiveGroupSelector(latencies(10.0, 10.0, 500.0), 1.2, 2.0, 1.0, random::nextDouble);
    int group0 = 0;
    int group1 = 0;
    int total = 6000;
  ...
  public void testFullSkewFavoursFastGroup() {
    // group0 = 10ms, group1 = 100ms => ratio 10 >= full-skew 2 => full skew. strength(0)=1/10, strength(1)=1/100, so
    // the fast group should take roughly 10/11 of the traffic. Fixed-seed RNG keeps this deterministic.
    Random random = new Random(7);
    LatencyAdaptiveGroupSelector selector =
        new LatencyAdaptiveGroupSelector(latencies(10.0, 100.0), 1.2, 2.0, 1.0, random::nextDouble);
    int fast = 0;
    int total = 8000;
    for (int i = 0; i < total; i++) {
