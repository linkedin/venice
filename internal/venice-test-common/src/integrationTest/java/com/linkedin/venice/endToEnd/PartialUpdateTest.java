...
public class PartialUpdateTest {
  ...
  public void testDebug() {
    ...
    try (ControllerClient parentControllerClient = new ControllerClient(CLUSTER_NAME, parentControllerUrl)) {
      ...
      UpdateStoreQueryParams updateStoreParams2 = new UpdateStoreQueryParams().setEnableWrites(false);
      ControllerResponse updateStoreResponse2 =
          parentControllerClient.retryableRequest(5, c -> c.updateStore(storeName, updateStoreParams2));
      assertFalse(updateStoreResponse2.isError(), "Update store got error: " + updateStoreResponse2.getError());
      ...
      try (AvroGenericStoreClient<Object, Object> storeReader = ClientFactory.getAndStartGenericAvroClient(
        ...
        TestUtils.waitForNonDeterministicAssertion(10, TimeUnit.SECONDS, true, () -> {
          try {
            GenericRecord value = readValue(storeReader, key);
            assertNotNull(value, "Key " + key + " should not be missing!");
            assertEquals(value.get("id").toString(), "101");
            assertEquals(value.get("name").toString(), "U.S. 101");
          } catch (Exception e) {
            throw new VeniceException(e);
          }
        });
