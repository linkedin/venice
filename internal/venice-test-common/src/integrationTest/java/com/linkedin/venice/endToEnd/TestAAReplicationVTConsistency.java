...
public class TestAAReplicationVTConsistency extends AbstractMultiRegionTest {
  ...
  private void verifyVTConsistency(
    ...
    VTConsistencyChecker.Snapshot dc0Snapshot =
        buildSnapshotForPartition(dc0Broker, new VeniceProperties(dc0Props), topicRepository, tp, "DC-0-vtConsumer");
    VTConsistencyChecker.Snapshot dc1Snapshot =
        buildSnapshotForPartition(dc1Broker, new VeniceProperties(dc1Props), topicRepository, tp, "DC-1-vtConsumer");
    ...
    for (VTConsistencyChecker.Inconsistency inc: inconsistencies) {
      switch (inc.type) {
        ...
        case VALUE_MISMATCH:
          ...
          System.out
              .printf("[INCONSISTENCY] key=%-12s  expected winner by logicalTimestamp: %s%n", inc.key, expectedWinner);
          System.out.printf(
    ...
    assertNotEquals(wolfInc.dc0Record.valueHash, wolfInc.dc1Record.valueHash, "DC-0 and DC-1 value hashes must differ");
