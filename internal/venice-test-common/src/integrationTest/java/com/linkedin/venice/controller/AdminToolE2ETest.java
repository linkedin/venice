...
public class AdminToolE2ETest {
  ...
  public void testSkipAdminMessage() throws Exception {
    ...
    String[] adminToolArgs1 = new String[] { "--execution-id", "10", "--url",
        childControllerClient.getLeaderControllerUrl(), "--cluster", clusterName, "--skip-admin" };
    AdminTool.main(adminToolArgs1);

    String[] adminToolArgs2 = new String[] { "--offset", "10", "--url", childControllerClient.getLeaderControllerUrl(),
        "--cluster", clusterName, "--skip-admin" };
    AdminTool.main(adminToolArgs2);

    try {
      String[] adminToolArgs3 = new String[] { "--url", childControllerClient.getLeaderControllerUrl(), "--cluster",
          clusterName, "--skip-admin" };
      AdminTool.main(adminToolArgs3);
      Assert.fail("Should have failed because neither execution id nor offset is specified.");
    } catch (RuntimeException e) {
      // expected
    }

    try {
      String[] adminToolArgs4 = new String[] { "--offset", "10", "--execution-id", "10", "--url",
          childControllerClient.getLeaderControllerUrl(), "--cluster", clusterName, "--skip-admin" };
      AdminTool.main(adminToolArgs4);
      Assert.fail("Should have failed because exactly one of execution id and offset should be specified.");
    } catch (RuntimeException e) {
      // expected
    }
  }
