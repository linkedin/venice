...
public class TestMaterializedViewEndToEnd {
  ...
  public void testLargeValuePushMaterializedViewCCConsumer()
    ...
    assertEquals(polledChangeEvents.size(), numberOfRecords);
  }
  ...
  private static Int2LongMap getNumberOfRecordsPerPartition(TopicManager topicManager, PubSubTopic topic) {
    Map<PubSubTopicPartition, PubSubPosition> topicPositions = topicManager.getEndPositionsForTopic(topic);
    System.out.println("### Latest positions for topic " + topic.getName() + ": " + topicPositions);
    ...
    System.out.println("### Partition record count for topic " + topic.getName() + ": " + partitionRecordCount);
