...
public class TestControllerGrpcEndpoints {
  ...
  @Test(timeOut = TIMEOUT_MS)
  public void testGetAllValueSchemaGrpcEndpoint() {
    String storeName = Utils.getUniqueString("test_get_all_value_schema_store");
    String valueSchema = "\"string\"";
    String controllerGrpcUrl = veniceCluster.getLeaderVeniceController().getControllerGrpcUrl();
    ManagedChannel channel = Grpc.newChannelBuilder(controllerGrpcUrl, InsecureChannelCredentials.create()).build();
    StoreGrpcServiceGrpc.StoreGrpcServiceBlockingStub storeBlockingStub = StoreGrpcServiceGrpc.newBlockingStub(channel);
    ...
    // Step 2: Get all value schemas
    GetAllValueSchemaGrpcRequest getAllSchemaRequest =
        GetAllValueSchemaGrpcRequest.newBuilder().setStoreInfo(storeGrpcInfo).build();

    GetAllValueSchemaGrpcResponse getAllSchemaResponse = storeBlockingStub.getAllValueSchema(getAllSchemaRequest);
    assertNotNull(getAllSchemaResponse, "Response should not be null");
    assertEquals(getAllSchemaResponse.getStoreInfo().getStoreName(), storeName);
    assertEquals(getAllSchemaResponse.getStoreInfo().getClusterName(), veniceCluster.getClusterName());
    assertEquals(getAllSchemaResponse.getSchemasCount(), 1, "Should have 1 schema");
    assertEquals(getAllSchemaResponse.getSchemas(0).getId(), 1);
    assertEquals(getAllSchemaResponse.getSchemas(0).getSchemaStr(), valueSchema);
  }
