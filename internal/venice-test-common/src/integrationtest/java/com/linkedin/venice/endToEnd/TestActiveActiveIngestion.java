...
public class TestActiveActiveIngestion {
  ...
  public void testAAIngestionWithStoreView(CompressionStrategy compressionStrategy) throws Exception {
    try {
      ...
      props.setProperty(
          ...
          Integer.toString(LARGE_RECORD_SIZE));
      ...
      storeParms = new UpdateStoreQueryParams().setStoreViews(viewConfig);
      updateStore(clusterName, props, storeParms);
      ...
      KafkaBrokerWrapper localKafka = multiRegionMultiClusterWrapper.getChildRegions().get(0).getKafkaBrokerWrapper();
  ...
  private int pollAfterImageEventsFromChangeCaptureConsumer(
    ...
    // Poll twice. We do this because we'll return a batch of events short of the timeout between version swaps. By
    // polling
    // twice we'll make sure we get all events on both sides of the swaps.
