...
public class VeniceClusterWrapper extends ProcessWrapper {
  ...
  static ServiceProvider<VeniceClusterWrapper> generateService(VeniceClusterCreateOptions options) {
    ...
    try {
      ...
      for (int i = 0; i < options.getNumberOfControllers(); i++) {
        if (options.getNumberOfRouters() > 0) {
          ClientConfig clientConfig = new ClientConfig().setVeniceURL(zkAddress)
              ...
              .setStoreName("dummy");
          options.getExtraProperties().put(VeniceServerWrapper.CLIENT_CONFIG_FOR_CONSUMER, clientConfig);
        }
      ...
      for (int i = 0; i < options.getNumberOfServers(); i++) {
        ...
        featureProperties.setProperty(
            VeniceServerWrapper.SERVER_ENABLE_SERVER_ALLOW_LIST,
            Boolean.toString(options.isEnableAllowlist()));
        featureProperties.setProperty(
            VeniceServerWrapper.SERVER_IS_AUTO_JOIN,
            Boolean.toString(options.isEnableAutoJoinAllowlist()));
        featureProperties
            .setProperty(VeniceServerWrapper.SERVER_ENABLE_SSL, Boolean.toString(options.isSslToStorageNodes()));
        featureProperties
            .setProperty(VeniceServerWrapper.SERVER_SSL_TO_KAFKA, Boolean.toString(options.isSslToKafka()));
        if (!veniceRouterWrappers.isEmpty()) {
          ClientConfig clientConfig = new ClientConfig().setVeniceURL(zkAddress)
              .setD2ServiceName(VeniceRouterWrapper.CLUSTER_DISCOVERY_D2_SERVICE_NAME)
              .setSslFactory(SslUtils.getVeniceLocalSslFactory());
          featureProperties.put(VeniceServerWrapper.CLIENT_CONFIG_FOR_CONSUMER, clientConfig);
        }
  ...
  public VeniceControllerWrapper addVeniceController(Properties properties) {
    VeniceControllerWrapper veniceControllerWrapper = ServiceFactory.getVeniceController(
        new VeniceControllerCreateOptions.Builder(clusterName, zkServerWrapper, kafkaBrokerWrapper).coloName(coloName)
  ...
  public VeniceServerWrapper addVeniceServer(boolean enableAllowlist, boolean enableAutoJoinAllowList) {
    Properties featureProperties = new Properties();
    featureProperties
        .setProperty(VeniceServerWrapper.SERVER_ENABLE_SERVER_ALLOW_LIST, Boolean.toString(enableAllowlist));
    featureProperties.setProperty(VeniceServerWrapper.SERVER_IS_AUTO_JOIN, Boolean.toString(enableAutoJoinAllowList));
    VeniceServerWrapper veniceServerWrapper = ServiceFactory.getVeniceServer(
        coloName,
        clusterName,
        kafkaBrokerWrapper,
