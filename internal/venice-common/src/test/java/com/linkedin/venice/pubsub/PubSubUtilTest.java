...
public class PubSubUtilTest {
  ...
  public void testGetBase64EncodedString() {
    ...
    assertEquals(encoded, "SGVsbG8sIFdvcmxkIQ==", "Should encode byte array to Base64 string");

    // Test with empty byte array
    byte[] emptyBytes = new byte[0];
    String encodedEmpty = PubSubUtil.getBase64EncodedString(emptyBytes);
    assertEquals(encodedEmpty, "", "Should encode empty byte array to empty string");

    // Test with null input
    String encodedNull = PubSubUtil.getBase64EncodedString(null);
    assertEquals(encodedNull, null, "Should return null for null input");
  ...
  public void testGetBase64DecodedBytes() {
    ...
    byte[] decoded = PubSubUtil.getBase64DecodedBytes(base64String);
    assertEquals(new String(decoded), "Hello, World!", "Should decode Base64 string to original bytes");
  ...
  public void testGetBase64DecodedBytesWithInvalidInput() {
    ...
    assertThrows(IllegalArgumentException.class, () -> {
      PubSubUtil.getBase64DecodedBytes("Invalid Base64!");
    });

    // Test with null input - this will throw NPE from Base64.getDecoder().decode()
    assertThrows(NullPointerException.class, () -> {
      PubSubUtil.getBase64DecodedBytes(null);
    });
  }
  ...
  public void testBase64RoundTrip() {
    ...
    byte[] originalData = "Test data for round-trip encoding/decoding with special chars: äöü!@#$%^&*()".getBytes();
