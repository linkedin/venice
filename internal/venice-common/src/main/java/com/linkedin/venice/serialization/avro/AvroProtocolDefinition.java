...
public enum AvroProtocolDefinition {
   ...
   *
   * New versions must be registered in this protocol's system store before writers emit them. Controllers configured
   * with a router client use a SchemaReader to resolve versions that are newer than their compiled protocol; other
   * deployments still require controllers to be upgraded first.
   */
