...
public class ApacheKafkaAdminConfig {
  ...
  public ApacheKafkaAdminConfig(PubSubAdminAdapterContext context) {
    ...
    this.defaultApiTimeout = Duration.ofMillis(defaultApiTimeoutInMs);
  ...
  public String toString() {
    return "ApacheKafkaAdminConfig{brokerAddress=" + adminProperties.get(AdminClientConfig.BOOTSTRAP_SERVERS_CONFIG)
        + ", securityProtocol=" + adminProperties.get(CommonClientConfigs.SECURITY_PROTOCOL_CONFIG) + ", clientId="
        + adminProperties.get(ConsumerConfig.CLIENT_ID_CONFIG) + ", defaultApiTimeout=" + defaultApiTimeout.toMillis()
        + "ms}";
  }
