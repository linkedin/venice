...
public enum PushJobCheckpoints implements VeniceEnumValue {
  ...
  START_JOB_STATUS_POLLING(4), JOB_STATUS_POLLING_COMPLETED(5), @Deprecated
  START_VALIDATE_SCHEMA_AND_BUILD_DICT_MAP_JOB(6), @Deprecated
  VALIDATE_SCHEMA_AND_BUILD_DICT_MAP_JOB_COMPLETED(7), START_VERSION_SWAP(8), COMPLETE_VERSION_SWAP(9),
  QUOTA_EXCEEDED(-1), WRITE_ACL_FAILED(-2), DUP_KEY_WITH_DIFF_VALUE(-3), INPUT_DATA_SCHEMA_VALIDATION_FAILED(-4),
  EXTENDED_INPUT_DATA_SCHEMA_VALIDATION_FAILED(-5), RECORD_TOO_LARGE_FAILED(-6), CONCURRENT_BATCH_PUSH(-7),
  DATASET_CHANGED(-8), INVALID_INPUT_FILE(-9), @Deprecated
  ZSTD_DICTIONARY_CREATION_FAILED(-10), DVC_INGESTION_ERROR_DISK_FULL(-11),
