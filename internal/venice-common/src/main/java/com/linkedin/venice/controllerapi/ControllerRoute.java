...
public enum ControllerRoute {
  ...
  REQUEST_TOPIC(
      "/request_topic", HttpMethod.POST, Arrays.asList(NAME, PUSH_TYPE, PUSH_JOB_ID),
      Collections.singletonList(PUSH_IN_SORTED_ORDER)
  ),
  ...
  NEW_STORE(
      "/new_store", HttpMethod.POST, Arrays.asList(NAME, KEY_SCHEMA, VALUE_SCHEMA),
      Arrays.asList(OWNER, IS_SYSTEM_STORE, ACCESS_PERMISSION)
  ),
  ...
  UPDATE_STORE(
      "/update_store", HttpMethod.POST, Collections.singletonList(NAME),
      Arrays.asList(
          OWNER,
          VERSION,
  ...
  ROLLBACK_TO_BACKUP_VERSION(
      "/rollback_to_backup_version", HttpMethod.POST, Collections.singletonList(NAME),
      Collections.singletonList(REGIONS_FILTER)
  ),
  AGGREGATED_HEALTH_STATUS(
      "/aggregatedHealthStatus", HttpMethod.POST, Collections.emptyList(), Collections.emptyList(), false
  ),
  ROLL_FORWARD_TO_FUTURE_VERSION(
      "/roll_forward_to_future_version", HttpMethod.POST, Collections.singletonList(NAME),
      Collections.singletonList(REGIONS_FILTER)
  ),
  ...
  UPDATE_CLUSTER_CONFIG(
      ...
      Collections.singletonList(SERVER_KAFKA_FETCH_QUOTA_RECORDS_PER_SECOND)
  ),
  ...
  LIST_STORES(
      "/list_stores", HttpMethod.GET, Collections.emptyList(), Collections.singletonList(INCLUDE_SYSTEM_STORES)
  ...
  ClUSTER_HEALTH_INSTANCES(
      "/cluster_health_instances", HttpMethod.GET, Collections.emptyList(),
      Collections.singletonList(ENABLE_DISABLED_REPLICAS)
  ...
  NODE_REMOVABLE(
      "/node_removable", HttpMethod.GET, Collections.singletonList(STORAGE_NODE_ID),
      Arrays.asList(INSTANCE_VIEW, TO_BE_STOPPED_INSTANCES)
  ...
  ADD_VALUE_SCHEMA(
      "/add_value_schema", HttpMethod.POST, Arrays.asList(NAME, VALUE_SCHEMA),
      Arrays.asList(SCHEMA_ID, SCHEMA_COMPAT_TYPE)
  ),
  ADD_DERIVED_SCHEMA(
      "/add_derived_schema", HttpMethod.POST, Arrays.asList(NAME, SCHEMA_ID, DERIVED_SCHEMA),
      Collections.singletonList(DERIVED_SCHEMA_ID)
  ...
  SEND_PUSH_JOB_DETAILS(
      "/send_push_job_details", HttpMethod.POST, Arrays.asList(CLUSTER, NAME, VERSION),
      Collections.singletonList(PUSH_JOB_DETAILS)
  ),
  ...
  GET_REPUSH_INFO(
      "/get_repush_info", HttpMethod.GET, Collections.singletonList(NAME), Collections.singletonList(FABRIC)
  ), WIPE_CLUSTER("/wipe_cluster", HttpMethod.POST, Arrays.asList(CLUSTER, FABRIC), Arrays.asList(NAME, VERSION)),
  ...
  PREPARE_DATA_RECOVERY(
      ...
      Collections.singletonList(AMPLIFICATION_FACTOR)
  ),
  IS_STORE_VERSION_READY_FOR_DATA_RECOVERY(
      "/is_store_version_ready_for_data_recovery", HttpMethod.GET,
      Arrays.asList(CLUSTER, SOURCE_FABRIC, FABRIC, NAME, VERSION), Collections.singletonList(AMPLIFICATION_FACTOR)
  ...
  GET_ADMIN_TOPIC_METADATA(
      "/get_admin_topic_metadata", HttpMethod.GET, Collections.singletonList(CLUSTER), Collections.singletonList(NAME)
  ),
  UPDATE_ADMIN_TOPIC_METADATA(
      "/update_admin_topic_metadata", HttpMethod.POST, Arrays.asList(CLUSTER, EXECUTION_ID),
      Arrays.asList(NAME, OFFSET, UPSTREAM_OFFSET)
  ...
  UPDATE_STORAGE_PERSONA(
      "/update_storage_persona", HttpMethod.POST, Arrays.asList(CLUSTER, PERSONA_NAME),
      Arrays.asList(PERSONA_QUOTA, PERSONA_STORES, PERSONA_OWNERS)
  ),
  ...
  DELETE_UNUSED_VALUE_SCHEMAS(
      ...
      Collections.singletonList(VALUE_SCHEMA_IDS)
  ...
  private final boolean bodyHasSensitiveContent;

  ControllerRoute(String path, HttpMethod httpMethod, List<String> params) {
    this(path, httpMethod, params, Collections.emptyList(), true);
  }

  ControllerRoute(String path, HttpMethod httpMethod, List<String> params, List<String> optionalParams) {
    this(path, httpMethod, params, optionalParams, true);
  }
  ...
  ControllerRoute(
      ...
      boolean bodyHasSensitiveContent) {
    this.path = path;
    this.httpMethod = httpMethod;
    this.params = params;
    this.optionalParams = optionalParams;
    this.bodyHasSensitiveContent = bodyHasSensitiveContent;
  }
  ...
  public boolean doesBodyHaveSensitiveContent() {
    return bodyHasSensitiveContent;
  }
}
