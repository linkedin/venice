...
public class StreamingResponseTrackerTest {
  ...
  public void testStreamingResponseTracker() {
    ...
    verifyRecordStreamingResponseTime(mockClientStats, true, false, false);
    for (int i = 0; i < 48; i++) {
      streamingResponseTracker.recordReceived();
    }
    verifyRecordStreamingResponseTime(mockClientStats, true, false, false);
    streamingResponseTracker.recordReceived();
    verifyRecordStreamingResponseTime(mockClientStats, true, true, false);
    for (int i = 0; i < 48; i++) {
      streamingResponseTracker.recordReceived();
    }
    verifyRecordStreamingResponseTime(mockClientStats, true, true, true);
    streamingResponseTracker.recordReceived();
    verifyRecordStreamingResponseTime(mockClientStats, true, true, true);
  }
