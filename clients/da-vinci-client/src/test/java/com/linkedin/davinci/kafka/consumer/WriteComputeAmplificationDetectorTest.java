...
public class WriteComputeAmplificationDetectorTest {
  ...
  public void testRecordBelowThresholdDoesNotTriggerReport() {
    ...
    // Should not report even after window elapsed
    assertFalse(detector.shouldReport(System.currentTimeMillis() + REPORT_INTERVAL_MS + 1));
  }
  ...
  public void testRecordAboveThresholdTriggersReport() {
    ...
    assertFalse(detector.shouldReport(startTime + 1000));

    // Should report after window elapsed
    assertTrue(detector.shouldReport(startTime + REPORT_INTERVAL_MS + 1));
  }

  @Test
  public void testBuildReportAndResetClearsState() {
    long startTime = System.currentTimeMillis();
    WriteComputeAmplificationDetector detector = new WriteComputeAmplificationDetector(REPORT_INTERVAL_MS);
    byte[] key = { 0x01 };
    ...
    long reportTime = startTime + REPORT_INTERVAL_MS + 1;
    assertTrue(detector.shouldReport(reportTime));

    WriteComputeAmplificationDetector.AmplificationReport report = detector.buildReportAndReset(reportTime);
    ...
    // After reset, should not report
    assertFalse(detector.shouldReport(reportTime + 1));
  }
  ...
  public void testMaxTrackedKeysLimit() {
    ...
    long reportTime = System.currentTimeMillis() + REPORT_INTERVAL_MS + 1;
    WriteComputeAmplificationDetector.AmplificationReport report = detector.buildReportAndReset(reportTime);
  ...
  public void testExistingKeyUpdatedWhenMapFull() {
    ...
    assertTrue(false, "Expected key 0x00 in top keys");
  }
  ...
  public void testReportToStringFormat() {
    ...
    assertTrue(output.contains("Large (>threshold): 1"));
  ...
  public void testWindowDurationInReport() {
    ...
    long reportTime = startTime + 90_000; // 90 seconds later
    WriteComputeAmplificationDetector.AmplificationReport report = detector.buildReportAndReset(reportTime);

    // Window duration should be approximately 90 seconds
    assertTrue(
        report.windowDurationMs >= 89_000 && report.windowDurationMs <= 91_000,
