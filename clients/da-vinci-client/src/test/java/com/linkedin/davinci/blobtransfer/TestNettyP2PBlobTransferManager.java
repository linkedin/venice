...
public class TestNettyP2PBlobTransferManager {
  ...
  public void testFailedRequestFromFinder() {
    ...
    Mockito.verify(versionedBlobTransferStats, Mockito.never())
        .recordBlobTransferRequest(anyString(), anyInt(), Mockito.anyBoolean(), Mockito.anyBoolean());
  }
  ...
  public void testNoResultFromFinder() {
    ...
    Mockito.verify(versionedBlobTransferStats).recordBlobTransferKafkaFallback(TEST_STORE, TEST_VERSION, true);
  }
  ...
  public void testLocalFileTransferInBatchStore()
    ...
    Mockito.verify(versionedBlobTransferStats).recordBlobTransferRequest(TEST_STORE, TEST_VERSION, true, true);
  }
  ...
  public void testFallsBackFromDaVinciPeerToServerAfterTransferFailure() throws Exception {
    ...
    transferOrder.verify(client)
        .get("server-host", TEST_STORE, TEST_VERSION, TEST_PARTITION, BlobTransferTableFormat.BLOCK_BASED_TABLE);
    Mockito.verify(versionedBlobTransferStats).recordBlobTransferRequest(TEST_STORE, TEST_VERSION, false, false);
    Mockito.verify(versionedBlobTransferStats).recordBlobTransferRequest(TEST_STORE, TEST_VERSION, true, true);
  }
  ...
  public void testAllPeersFailException() {
    ...
    Mockito.verify(versionedBlobTransferStats).recordBlobTransferKafkaFallback(TEST_STORE, TEST_VERSION, false);
  }
