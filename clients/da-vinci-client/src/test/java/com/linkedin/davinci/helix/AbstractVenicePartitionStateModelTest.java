...
public abstract class AbstractVenicePartitionStateModelTest<MODEL_TYPE extends AbstractPartitionStateModel, NOTIFIER_TYPE extends LeaderFollowerIngestionProgressNotifier> {
  ...
  public void testGetStoreType() {
    ...
    VeniceUserStoreType storeType = testStateModel.getStoreType();
    assertEquals(storeType, VeniceUserStoreType.SYSTEM, "Case 1: System store");
    ...
    assertEquals(storeType, VeniceUserStoreType.HYBRID_ONLY, "Case 2: Hybrid store");
    ...
    storeType = freshModel.getStoreType();
    assertEquals(storeType, VeniceUserStoreType.BATCH_ONLY, "Case 3: Batch store");

    // Case 4: Store not found returns ALL
    freshModel = getParticipantStateModel();
    when(mockReadOnlyStoreRepository.getStore(storeName)).thenReturn(null);
    storeType = freshModel.getStoreType();
    assertEquals(storeType, VeniceUserStoreType.ALL, "Case 4: Store not found");
  }
  ...
  public void testGetReplicaTypeDescription() {
    ...
    when(mockReadOnlyStoreRepository.getStore(storeName)).thenReturn(null);
    description = freshModel.getReplicaTypeDescription();
    assertEquals(description, "Store unknown version", "Case 4: Unknown store");
  }
}
