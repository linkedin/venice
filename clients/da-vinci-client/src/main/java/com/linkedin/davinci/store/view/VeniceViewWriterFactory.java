...
public class VeniceViewWriterFactory {
  ...
  public Map<String, VeniceViewWriter> buildStoreViewWriters(Store store, int version) {
    ...
    for (Map.Entry<String, ViewConfig> viewConfig: storeVersion.getViewConfigs().entrySet()) {
      ...
      VeniceViewWriter viewWriter =
          ViewWriterUtils.getVeniceViewWriter(className, properties, store, version, extraParams, veniceWriterFactory);
