...
public class PartitionConsumptionState {
  ...
  public PartitionConsumptionState(
    ...
    if (!Version.isATopicThatIsVersioned(partitionReplica.getTopicName())) {
      throw new IllegalArgumentException(
          "PartitionConsumptionState should be created only for versioned topic, but got: "
  ...
  public PubSubTopicPartition getSourceTopicPartition(PubSubTopic topic) {
    if (getTopicPartition().getPubSubTopic().equals(topic)) {
  ...
  public String getReplicaId() {
    return getTopicPartition().toString();
  }
  ...
  public void enableKeyUrnCompressionUponStartOfPush(List<String> keyUrnFields) {
    if (keyUrnCompressor == null) {
      LOGGER.info("Enabling key urn compression for replicaId: {} with fields: {}", getTopicPartition(), keyUrnFields);
    } else {
      LOGGER.info("Previous key urn compression dict will be overridden for replicaId: {}", getTopicPartition());
    }
