...
public class SystemStoreFreeMetadata extends AbstractStoreMetadata {
  ...
  public SystemStoreFreeMetadata(ClientConfig clientConfig) {
    super(clientConfig);
    ...
    this.d2TransportClient = ClientFactory
        .getTransportClient(com.linkedin.venice.client.store.ClientConfig.defaultGenericClientConfig(getStoreName()));
  ...
  @Override
  public void start() {
    // perform cluster discovery work upfront
  ...
  private void discoverD2Service(boolean retryOnFailure) {
    ...
    synchronized (this) {
      ...
      if (d2TransportClient instanceof D2TransportClient) {
        ...
        serviceName = new D2ServiceDiscovery().find(client, storeName, retryOnFailure).getD2Service();
  ...
  private CompletableFuture<TransportClientResponse> fetchMetadata() {
    ...
    String url = serviceName + "/" + QueryAction.METADATA.toString().toLowerCase() + "/" + storeName;
  ...
  private CompletableFuture<TransportClientResponse> fetchCompressionDictionary(int version) {
    ...
    String url = serviceName + "/" + QueryAction.DICTIONARY.toString().toLowerCase() + "/" + storeName + "/" + version;
