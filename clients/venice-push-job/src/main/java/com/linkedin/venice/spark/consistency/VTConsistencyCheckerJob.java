...
public class VTConsistencyCheckerJob {
  ...
  static final StructType OUTPUT_SCHEMA = new StructType(
      new StructField[] { new StructField("version_topic", DataTypes.StringType, false, Metadata.empty()),
          ...
          new StructField("dc0_offset_vector", new ArrayType(DataTypes.StringType, false), true, Metadata.empty()),
          new StructField("dc1_offset_vector", new ArrayType(DataTypes.StringType, false), true, Metadata.empty()),
          new StructField("dc0_high_watermark", new ArrayType(DataTypes.StringType, false), true, Metadata.empty()),
          new StructField("dc1_high_watermark", new ArrayType(DataTypes.StringType, false), true, Metadata.empty()),
  ...
  private static Iterator<Row> findInconsistenciesForPartition(
    ...
    try {
      ...
      PubSubTopicPartition topicPartition =
          new PubSubTopicPartitionImpl(topicRepository.getTopic(versionTopic), partition);
      ...
      try (TopicManager dc0TopicManager = createTopicManager(dc0Props, topicRepository, positionTypeRegistry);
        ...
        try (PubSubSplitIterator dc0Iterator = new PubSubSplitIterator(
            ...
            dc0Split,
            false)) {
          dc0Snapshot = VTConsistencyChecker.buildSnapshot(dc0Iterator, dc0TopicManager, positionDeserializer);
        }
        ...
        try (PubSubSplitIterator dc1Iterator = new PubSubSplitIterator(
            ...
            dc1Split,
            false)) {
          dc1Snapshot = VTConsistencyChecker.buildSnapshot(dc1Iterator, dc1TopicManager, positionDeserializer);
        }

        List<VTConsistencyChecker.Inconsistency> found = VTConsistencyChecker
            .findInconsistencies(dc0Snapshot, dc1Snapshot, dc0TopicManager, dc1TopicManager, topicPartition);
  ...
  private static Row toRow(VTConsistencyChecker.Inconsistency inc, String versionTopic, int partition) {
    ...
    String[] dc0Ov = inc.dc0Record != null ? toStringArray(inc.dc0Record.upstreamRTOffset) : null;
    String[] dc1Ov = inc.dc1Record != null ? toStringArray(inc.dc1Record.upstreamRTOffset) : null;
    String[] dc0Hw = inc.dc0Record != null ? toStringArray(inc.dc0Record.highWatermark) : null;
    String[] dc1Hw = inc.dc1Record != null ? toStringArray(inc.dc1Record.highWatermark) : null;
  ...
  private static String[] toStringArray(List<PubSubPosition> positions) {
    String[] arr = new String[positions.size()];
    for (int i = 0; i < positions.size(); i++) {
      PubSubPosition pos = positions.get(i);
      arr[i] = pos != null ? pos.toString() : null;
    }
    return arr;
  }
