...
import static com.linkedin.venice.ConfigKeys.PUSH_JOB_FLINK_MATERIALIZED_VIEW_ENABLED;
...
public class DataWriterMRJob extends DataWriterComputeJob {
  ...
  private void setupDefaultJobConf(JobConf conf, PushJobSetting pushJobSetting, VeniceProperties props) {
    ...
    if (pushJobSetting.sslToKafka) {
      conf.set(PUBSUB_SECURITY_PROTOCOL, PubSubSecurityProtocol.SSL.name());
  ...
  private void setupReducerConf(JobConf jobConf, PushJobSetting pushJobSetting) {
    ...
    if (pushJobSetting.materializedViewConfigFlatMap != null) {
      ...
      jobConf.set(
          PUSH_JOB_FLINK_MATERIALIZED_VIEW_ENABLED,
          Boolean.toString(pushJobSetting.isFlinkMaterializedViewEnabled));
