package com.linkedin.venice.hadoop.recordreader.vson;
...
public class VeniceVsonRecordReader extends AbstractVeniceRecordReader<BytesWritable, BytesWritable> {
  ...
  public Iterator<Pair<byte[], byte[]>> iterator() {
    ...
    return new VsonIterator(fileReader, this);
  }
  ...
  private static class VsonIterator implements Iterator<Pair<byte[], byte[]>> {
    ...
    public VsonIterator(SequenceFile.Reader fileReader, VeniceVsonRecordReader recordReader) {
    ...
    public Pair<byte[], byte[]> next() {
      ...
      byte[] keyBytes = recordReader.getKeyBytes(currentKey, currentValue);
      byte[] valueBytes = recordReader.getValueBytes(currentKey, currentValue);
