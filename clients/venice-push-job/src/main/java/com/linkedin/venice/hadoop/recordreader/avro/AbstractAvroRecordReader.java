...
public abstract class AbstractAvroRecordReader<INPUT_KEY, INPUT_VALUE>
  ...
  private static final Logger LOGGER = LogManager.getLogger(AbstractAvroRecordReader.class);
   ...
   */
  public AbstractAvroRecordReader(VeniceProperties props) {
  ...
  public Iterator<Pair<byte[], byte[]>> iterator() {
    ...
    return new AbstractAvroRecordReader.AvroIterator(avroDataFileStream, this);
  }
  ...
  private static class AvroIterator implements Iterator<Pair<byte[], byte[]>> {
    ...
    private final AbstractAvroRecordReader recordReader;

    public AvroIterator(DataFileStream avroDataFileStream, AbstractAvroRecordReader recordReader) {
    ...
    public Pair<byte[], byte[]> next() {
      ...
      try {
        ...
        byte[] keyBytes = recordReader.getKeyBytes(hadoopKey, hadoopValue);
        byte[] valueBytes = recordReader.getValueBytes(hadoopKey, hadoopValue);
