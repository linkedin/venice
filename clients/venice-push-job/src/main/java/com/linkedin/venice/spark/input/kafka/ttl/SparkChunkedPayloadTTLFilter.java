...
public class SparkChunkedPayloadTTLFilter implements Serializable, AutoCloseable {
  ...
  /**
   * Check if the assembled row should be filtered based on TTL.
   *
   * @param assembledRow Row with DEFAULT_SCHEMA_WITH_SCHEMA_ID (key, value, rmd, schema_id, rmd_version_id)
   * @return true if the row should be filtered (removed), false if it should be kept
   */
  public boolean shouldFilter(Row assembledRow) throws IOException {
    return getTTLFilter().checkAndMaybeFilterValue(new AssembledRowWrapper(assembledRow));
  }
