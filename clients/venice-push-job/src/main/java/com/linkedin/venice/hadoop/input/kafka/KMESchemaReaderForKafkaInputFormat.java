...
public class KMESchemaReaderForKafkaInputFormat implements SchemaReader {
  ...
  public KMESchemaReaderForKafkaInputFormat(Map<Integer, String> newerKmeSchemas) {
    ...
    SchemaData schemaData = new SchemaData(AvroProtocolDefinition.KAFKA_MESSAGE_ENVELOPE.getSystemStoreName(), null);
    ...
    for (Map.Entry<Integer, Schema> entry: allSchemasFromResources.entrySet()) {
      schemaData.addValueSchema(new SchemaEntry(entry.getKey(), entry.getValue().toString()));
    }
   ...
   * @throws UnsupportedOperationException Always thrown as update schemas are not supported
   */
  @Override
