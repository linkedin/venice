...
public class VTConsistencyCheckerTest {
  ...
  public void testFindInconsistenciesSkipsRecordsWhenOneDCHadIncompleteInfo() {
    VTConsistencyChecker.KeyRecord dc0Wolf = new VTConsistencyChecker.KeyRecord(
        ...
        200L,
        "pos:42");
    VTConsistencyChecker.KeyRecord dc1Wolf =
        new VTConsistencyChecker.KeyRecord("dire-wolf", Arrays.asList(5L, 3L), Arrays.asList(20L, 8L), 180L, "pos:17");
  ...
  public void testBuildSnapshotEmptyPartitionReturnsEmptySnapshot() {
    ...
    try (PubSubSplitIterator iterator = new PubSubSplitIterator(consumer, split, false)) {
      VTConsistencyChecker.Snapshot snapshot = VTConsistencyChecker.buildSnapshot(iterator);

      assertTrue(snapshot.keyRecords.isEmpty(), "Empty partition must yield no key records");
      assertEquals(
          snapshot.partitionHighWatermark,
          Arrays.asList(-1L, -1L),
          "HW must remain all -1 for empty partition");
   ...
   *   <li>OV        = [42, -1]   (colo-0 seen, colo-1 not yet)</li>
   *   <li>HW        = [42, -1]   (running max same as OV after first message)</li>
   *   <li>logicalTs = 100</li>
  ...
  public void testBuildSnapshotCapturesSinglePutMessage() {
    ...
    assertEquals(rec.upstreamRTOffset, Arrays.asList(42L, -1L));
    assertEquals(rec.highWatermark, Arrays.asList(42L, -1L));
   ...
   *
   * <p>msg0: key="hawk", coloId=0, upstreamOffset=10 → HW after = [10, -1]
   * <p>msg1: key="wolf", coloId=1, upstreamOffset=20 → HW after = [10, 20]
   *
   * <p>hawk's record must carry HW=[10,-1]; wolf's record must carry HW=[10,20].
   */
  @Test
  public void testBuildSnapshotRunningHighWatermarkUpdatesPerMessage() {
    ...
    assertEquals(hawkRec.highWatermark, Arrays.asList(10L, -1L), "hawk HW must reflect only colo-0 seen so far");
    assertEquals(wolfRec.highWatermark, Arrays.asList(10L, 20L), "wolf HW must reflect both colos seen");
    assertEquals(
        snapshot.partitionHighWatermark,
  ...
  private VTConsistencyChecker.Snapshot buildSnapshotFromMessages(List<DefaultPubSubMessage> messages) {
    ...
    try (PubSubSplitIterator iterator = new PubSubSplitIterator(consumer, split, false)) {
      return VTConsistencyChecker.buildSnapshot(iterator);
    }
  }
