...
public class VTConsistencyCheckerJobTest {
  ...
  public void testToRowValueMismatchPopulatesAllFields() throws Exception {
    KeyRecord dc0 = new KeyRecord("arctic-wolf", Arrays.asList(5L, 10L), Arrays.asList(50L, 60L), 200L, "pos:42");
    KeyRecord dc1 = new KeyRecord("dire-wolf", Arrays.asList(10L, 15L), Arrays.asList(20L, 30L), 180L, "pos:17");
    ...
    assertEquals(row.get(6), new long[] { 5L, 10L }, "dc0_offset_vector");
    assertEquals(row.get(7), new long[] { 10L, 15L }, "dc1_offset_vector");
    assertEquals(row.get(8), new long[] { 50L, 60L }, "dc0_high_watermark");
    assertEquals(row.get(9), new long[] { 20L, 30L }, "dc1_high_watermark");
  ...
  public void testOutputSchemaContractIsStable() {
    ...
    assertField(6, "dc0_offset_vector", new ArrayType(DataTypes.LongType, false), true);
    assertField(7, "dc1_offset_vector", new ArrayType(DataTypes.LongType, false), true);
    assertField(8, "dc0_high_watermark", new ArrayType(DataTypes.LongType, false), true);
    assertField(9, "dc1_high_watermark", new ArrayType(DataTypes.LongType, false), true);
