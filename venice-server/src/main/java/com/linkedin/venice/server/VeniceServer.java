...
public class VeniceServer {
	...
	public static void main(String args[]) throws Exception {
		...
		try {
			if (args.length == 0) {
				...
				Utils.croak("USAGE: java " + VeniceServer.class.getName()
						+ " [venice_home_dir]");
			}
		} catch (Exception e) {
	...
	private VeniceStorageNode createNewStoreNode(int nodeId) {
		...
		case BDB:
			throw new UnsupportedOperationException(
					"BDB storage not yet implemented");
		case VOLDEMORT:
			throw new UnsupportedOperationException(
					"Voldemort storage not yet implemented");
		default:
			toReturn = new InMemoryStorageNode(nodeId);
			break;
		}
	...
	public Object readValue(String key) {
		throw new UnsupportedOperationException(
				"Read protocol is not yet supported");
	}
	...
	private List<Integer> getNodeMappings(int partitionId)
		...
		if (0 == numNodes) {
			throw new VeniceStorageException(
					"Cannot calculate node id for partition because there are no nodes!");
		}
