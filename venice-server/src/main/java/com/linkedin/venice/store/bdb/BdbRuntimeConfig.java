...
public class BdbRuntimeConfig {
  ...
  public BdbRuntimeConfig(BdbServerConfig bdbServerConfig) {
    LockMode lockMode = bdbServerConfig.getBdbReadUncommitted() ? LockMode.READ_UNCOMMITTED
      : LockMode.DEFAULT;
    setLockMode(lockMode);
    setStatsCacheTtlMs(bdbServerConfig.getBdbStatsCacheTtlMs());
    setExposeSpaceUtil(bdbServerConfig.getBdbExposeSpaceUtilization());
    setMinimizeScanImpact(bdbServerConfig.getBdbMinimizeScanImpact());
    setCheckpointerOffForBatchWrites(bdbServerConfig.getBdbCheckpointerOffForBatchWrites());
  }
