...
public class BackupVersionOptimizationServiceTest {
  ...
  private ReadOnlyStoreRepository mockStoreRepository(String storeName, int currentVersion, Map<Integer, VersionStatus> versionStatusMap) {
  ...
  public void testBackupVersionShouldBeOptimizedAfterBecomingInactive() throws Exception {
    ...
    BackupVersionOptimizationService optimizationService = new BackupVersionOptimizationService(storeRepository,
        storageEngineRepository, NO_READ_THRESHOLD_MS_FOR_DATABASE_OPTIMIZATION, 1,
    ...
    try {
      TestUtils.waitForNonDeterministicAssertion(10, TimeUnit.SECONDS, () -> verify(backupStorageEngine).reopenStoragePartition(
          PARTITION_ID_0));
    } finally {
  ...
  public void testBackupVersionShouldNotBeOptimizedIfNoRead() throws Exception {
    ...
    optimizationService.start();
    try {
      TestUtils.waitForNonDeterministicAssertion(3, TimeUnit.SECONDS, () -> verify(storageEngineRepository, atLeastOnce()).getAllLocalStorageEngines());
      TestUtils.waitForNonDeterministicAssertion(3, TimeUnit.SECONDS, () -> verify(storeRepository, atLeastOnce()).getStore(storeName));
