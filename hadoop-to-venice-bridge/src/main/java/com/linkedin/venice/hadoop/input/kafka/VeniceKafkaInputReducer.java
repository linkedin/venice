...
public class VeniceKafkaInputReducer extends VeniceReducer {
  ...
  @Override
  protected Optional<VeniceWriterMessage> extract(BytesWritable key, Iterator<BytesWritable> valueIterator, Reporter reporter) {
    /**
  ...
  private List<KafkaInputMapperValue> getMapperValues(Iterator<BytesWritable> valueIterator) {
    ...
    while (valueIterator.hasNext()) {
      mapperValues.add(KAFKA_INPUT_MAPPER_VALUE_AVRO_SPECIFIC_DESERIALIZER.deserialize(valueIterator.next().copyBytes()));
    }
    return mapperValues;
  }
}
