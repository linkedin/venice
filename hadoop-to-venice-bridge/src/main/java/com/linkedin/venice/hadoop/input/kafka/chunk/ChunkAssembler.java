...
public class ChunkAssembler {
    ...
    for (int i = values.size() - 1; i >= 0; i--) { // Start from the value with the highest offset
      ...
      if (isRegularMessage(mapperValue)) {
        return mapperValue.valueType == MapperValueType.DELETE ?
            Optional.empty() : Optional.of(new ValueBytesAndSchemaId(mapperValue.value, mapperValue.schemaId));

      } else if (mapperValue.schemaId == AvroProtocolDefinition.CHUNKED_VALUE_MANIFEST.getCurrentProtocolVersion()) { // chunk manifest
        ...
        throw new VeniceException(String.format("Unhandled case with chunked key suffix %s and value schema ID %d",
            deserializeChunkedKeySuffix(mapperValue.chunkedKeySuffix), mapperValue.schemaId));
      }
    }
  ...
  private ValueBytesAndSchemaId assembleLargeValue(final ChunkedValueManifest manifest) {
    ...
    for (int chunkIndex = 0; chunkIndex < manifest.keysWithChunkIdSuffix.size(); chunkIndex++) {
      ...
      if (chunkBytes == null) {
        throw new VeniceException("Cannot assemble a large value. Missing a chunk with the composite key: " +
  ...
  private void buildChunksByCompositeKeyMapping(final byte[] keyBytes, final List<KafkaInputMapperValue> values) {
    values.forEach(mapperValue -> {
      if (mapperValue.schemaId == AvroProtocolDefinition.CHUNK.getCurrentProtocolVersion()) {
        ByteBuffer compositeKey = createCompositeKey(keyBytes, ByteUtils.extractByteArray(mapperValue.chunkedKeySuffix));
